...
def sorted_cases(
    ...
    sorted_rows.sort(
        key=lambda row: (
            normalize(row.get("work_order"), "work_order"),
            row.get("id", 0),
        )
    )
    
    sorted_rows.sort(
        key=lambda row: normalize(row.get(sort_key), sort_key),
        reverse=reverse,
    )
...
def cases_redirect_url(
    ...
    params = {
        ...
        "followups_only": str(followups_only).lower(),
    }

    return "/cases?" + urlencode(params)


def safe_cases_return_url(return_to: str | None) -> str:
    """Allow redirects only back into the local Cases page."""
    if return_to and (return_to == "/cases" or return_to.startswith("/cases?")):
        return return_to
    return cases_redirect_url(sort="work_order", direction="asc")
...
def cases(
    ...
    return template(
        ...
        "cases.html",
        {
            ...
            "sort": sort,
            "direction": direction,
            "current_cases_url": current_cases_url,
...
def update_status(
    ...
    return RedirectResponse(
        safe_cases_return_url(return_to),
        status_code=303,
    )
